...
VkResult
wsi_arlinux_connect(struct wsi_arlinux_chain *chain, struct wl_display *display)
{
...
void
wsi_arlinux_disconnect(struct wsi_arlinux_chain *chain)
{
...
buffer_release(void *data, struct wl_buffer *buffer)
{
   ...
   if (image->state == ARLINUX_PRESENTED)
...
{
   struct wsi_arlinux_image *image = data;
   if (image->num_fds == ARLINUX_MAX_FDS) {
      close(fd);
...
VkResult
wsi_arlinux_alloc_buffer(struct wsi_arlinux_chain *chain, struct wsi_arlinux_image *image)
{
   image->chain = chain;
   struct android_wlegl_server_buffer_handle *handle =
      android_wlegl_get_server_buffer_handle(chain->wlegl, chain->width,
         chain->height, chain->format, ARLINUX_BUFFER_USAGE);
   if (!handle)
      return VK_ERROR_OUT_OF_HOST_MEMORY;
...
void
wsi_arlinux_free_buffer(struct wsi_arlinux_image *image)
{
...
VkResult
wsi_arlinux_dispatch(struct wsi_arlinux_chain *chain, uint64_t timeout)
{
   ...
   while (wl_display_prepare_read_queue(chain->display, chain->queue) != 0) {
      ...
      if (dispatched) {
         if (chain->connection) wsi_arlinux_x11_drain(chain);
         return chain->status;
      }
   }
   struct pollfd fds[2] = {
...
VkResult
wsi_arlinux_wayland_present(struct wsi_arlinux_chain *chain,
                          struct wsi_arlinux_image *image)
{
   /* FIFO: never overwrite a commit waiting for the compositor's next frame. */
   while (chain->frame) {
      VkResult result = wsi_arlinux_dispatch(chain, UINT64_MAX);
